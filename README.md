...
![Eighteen X-plug attachment variants: three plates, five vertical tile brackets, eight normal full-solid stops and two angled stops, each individually labeled.](docs/images/x-attachments.png)

The complete library contains **56 accessory variants** for the documented **350x320x325 mm catalogue envelope**, original joints and default accessory parameters. **[Browse all 56 parts, each with its own thumbnail beside its name, dimensions and purpose](docs/attachments.md).** The inventory is grouped by family: floor ramps, plates, vertical tile brackets, normal full-solid stops, angled stops, edge strips, inner/outer corners, support rails, rail ends and connectors.
...
Open **`outputs/full-catalogue/job.3mf`** as a project. This H2D-specific plan currently contains **81 designs on 24 family-grouped plates**: 25 ordered tile sizes and 56 accessories, with no omissions. Ordinary plates keep every visible model inside the verified dual-nozzle common reach X=25..325, Y=0..320 and Z<=320, inset another 5 mm, with at least 10 mm between model bounds. The 306x306 mm 5x5 tile cannot fit the 300 mm common width, so its isolated plate is labeled **`5x5 TILE - SINGLE NOZZLE ONLY - LEFT`** and explicitly maps filament slot 1 to the left nozzle. Other plates keep automatic Convenience Mode. The two bracket plates and Bambu object list use the plain floor-base/wall names below; stable IDs remain in the manifest and placement records. The manifest is the authoritative object/plate inventory and records the exception, family groups, ramp/normal-stop/shallow-bracket support settings and accessory poses.
...
![Five tile brackets with separate ordinary floor and wall tiles: three depth-matched originals plus shallow floor 1x1 to wall 1x2 and floor 2x1 to wall 2x2 variants.](docs/images/vertical-tile-brackets.png)

Name brackets by both footprints **in use**, not by the rotated print pose. The three original variants are **Deep tall — floor 1x2, wall 1x2**, **Wide low — floor 2x1, wall 2x1** and **Deep square — floor 2x2, wall 2x2**. The two shallow variants are **Shallow tall — floor 1x1, wall 1x2** and **Shallow wide — floor 2x1, wall 2x2**. Width is left-right X cells, `--depth-cells` is floor-base Y rows, and `--panel-height-cells` independently selects upright wall Z rows; omitting panel height preserves the original behavior of matching floor depth.
...
This exports **only the bracket**. Generate floor and wall tiles separately with their stated dimensions. The five catalogue configurations are floor 1x2 -> wall 1x2, floor 2x1 -> wall 2x1, floor 2x2 -> wall 2x2, floor 1x1 -> wall 1x2 and floor 2x1 -> wall 2x2. Omitting all cell options chooses the backward-compatible floor/wall 2x1 variant. To generate the shallow tall bracket, use `--width-cells 1 --depth-cells 1 --panel-height-cells 2`; the shallow wide bracket uses width 2 with the same one-row base and two-row wall. They require the standard 60 mm pitch, 13 mm tile height and zero fit offset. The unreleased `lock-90` family is replaced by `vertical-tile-bracket`, without an alias; `--stop-height-mm` applies to normal `vertical-stop` and angled `lock-45` stops, not these brackets.
...
**Bambu projects rotate original brackets X=135 degrees diagonal-face-down, shallow brackets Y=-90 degrees broad-side-down, normal stops by their per-design broad-rear-face-down angle and angled stops X=-135 degrees back-face-down before fit checks and plate packing.** This applies to both single-part and catalogue `--bambu` jobs. STEP, STL and core 3MF keep their model datums; when using those files directly, apply the manifest's recommended print pose in your slicer. Normal Auto support is scoped to shallow-bracket and normal-stop objects without globally enabling it for other bracket/tile entries. In both documented H2D PETG profiles, shallow-bracket support reached the floor and wall X mating regions; remove it completely from the exposed lobes and roots before fit checks. Always inspect bed contact, actual supports, material assignments and warnings; an unsliced project is not print approval.

Edges and corners have selective R2 free top rims. Rails use smaller selective R1 rounds. Original bracket front lips use R1 outside the tile's planar bearing land. Each shallow bracket couples R1 across its 20 free exterior body edges while retaining the necessary internal panel-bearing corner and exact X profiles; its filled backing covers the upper connector roots rather than letting them break through the rear surface. Angled stops are full-width filled CAD wedges, not open-backed wall/web assemblies: one coupled operation applies R1 to all 18 free body edges while exact X plugs and roots remain unchanged.
...
Normal Auto support is scoped to each `vertical-stop` object without globally enabling support for tiles, original brackets or other catalogue entries. In the documented H2D PETG CLI checks, 1x1/H120 and 2x1/H120 generated support reaching the mounting region; inspect the actual sliced paths and remove support completely before checking fit. Other tested variants generated no Auto support, but profile changes can change that result. These observations are not physical print, removal, strength or load verification.
...
Use `accessory_design(Accessory("vertical-tile-bracket", nx=1, ny=2))` from `cargo_grid.catalogue` / `cargo_grid.accessories` for the original deep tall bracket. Use `Accessory("vertical-tile-bracket", nx=1, ny=1, panel_height_cells=2)` for the shallow tall bracket; the shallow wide form uses `nx=2` with the same base depth and panel height. For a Bambu catalogue, call `catalogue_job(build, orient_for_bambu=True)` so eligibility uses the same transformed bounds as Bambu packing; the CLI does this automatically with `--bambu`. Custom-interface catalogues retain other supported families but omit the reference-only bracket family.
