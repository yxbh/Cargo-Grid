...
Do not include local locks, reference meshes, system profile JSON, `.venv`, `.local`, study outputs, G-code, unreviewed screenshots or runtime diagnostics. The four explicitly allowlisted, source-derived documentation composites under `docs/images/` are the intentional exception; verify their complete inventory, source/tool provenance, links, dimensions and size with `tools/render_docs.py --check`. They belong in the source distribution, not the runtime wheel. Publication, tags and pushes remain separate explicit maintainer decisions; this checklist does not perform them.
