...
`vertical-tile-bracket` replaces the unreleased `lock-90` catalogue family without retaining an alias. Its supported grids are 1x2, 2x1 and 2x2 at the reference 60 mm pitch, 13 mm tile height and zero fit offset. In use, these are the Tall 1-column x 2-row bracket with a nominal 60x120 mm panel, Wide 2-column x 1-row bracket with a nominal 120x60 mm panel, and Square 2-column x 2-row bracket with a nominal 120x120 mm panel, excluding joining tabs. The first stable-ID count is base X / panel width; the second is base Y / panel height, regardless of the rotated print pose. Both joint styles retain the same bracket X geometry, but original tile-edge joins remain the recommended default and the existing full-height limitations still apply.
