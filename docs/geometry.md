...
`vertical-tile-bracket` replaces the unreleased `lock-90` catalogue family without retaining an alias. At the reference 60 mm pitch, 13 mm tile height and zero fit offset, it supports three depth-matched configurations—floor 1x2 -> wall 1x2, floor 2x1 -> wall 2x1 and floor 2x2 -> wall 2x2—and two independent-height configurations—floor 1x1 -> wall 1x2 and floor 2x1 -> wall 2x2. API `nx` is base/wall X width, `ny` is floor-base Y depth, and optional `panel_height_cells` is wall Z height; omitting or explicitly matching it to `ny` preserves the original specification and stable ID. Both joint styles retain the same bracket X geometry, but original tile-edge joins remain the recommended default and the existing full-height limitations still apply.
...
The two shallow variants retain a 60 mm floor depth, panel seat Y=47, panel plug tips Y=59.8 and the original two-row wall Z grid. Their natural filled rear profile rises 107 mm across the available 42.8 mm upper run rather than forcing the original 45-degree slope. The upper rear body stays behind the connector root with at least 1.78 mm cover, eliminating the former root break-through while preserving zero Boolean difference in both floor and panel X external regions. One coupled operation applies R1 to the 20 free exterior body edges; the internal panel-bearing corner and X mating profiles remain exact. Source bounds are 60x60x140.378 mm and 120x60x140.378 mm including downward plugs.
...
Ramps require original roofed joints at 60 mm pitch, 13 mm height and zero fit offset. Experimental full-height ramp geometry was rejected because a generic post-fillet cutter left real overlap with its matching tile; full-height/custom-interface catalogues omit ramps rather than claiming compatibility. There are no corner ramps, male adapters, X plugs, holes or added mechanisms.
...
Angled `lock-45` stops retain a 4.1 mm front base/toe, a 6 mm horizontal cap thickness (about 4.243 mm normal to the 45-degree cargo face), unchanged outer bounds and the exact existing X connectors/roots. The approved body is one full-width filled CAD wedge, not an open-backed wall with side webs. One coupled operation applies R1 to all 18 free envelope edges before restoring only the protected connector/root cores. The 1x1 and 2x2 Bambu poses remain X=-135 degrees; bounded native 0.8/0.32 and 0.4/0.20 checks produced no support with either OFF or Auto, which is slicer evidence rather than physical print approval.
