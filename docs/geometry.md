...
Straight rail and connector outer bodies use R3, with R3 window corners and R2 window rims. Rail-end variants 1/2 are more constrained: variant 1 uses R0.75 on the body, R2.5 on window corners and R1 on window rims; variant 2 uses R1/R3/R2. Variants 3/4 use R3/R3/R2. These combinations preserve the support dovetails and satisfy both default and adaptive STEP-volume checks on macOS and Linux. Male/female support tools are applied afterward. Attachment plates use R2 around the complete 4.1 mm body, then receive exact X plugs and R2 roots.
