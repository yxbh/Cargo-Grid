...
Rail and connector outer bodies use R3. Window corners use R3 and window rims use R2, except the acute first rail-end profile where the valid rim limit is R1.25. Male/female support dovetails are applied afterward from the shared exact tool. Attachment plates use R2 around the complete 4.1 mm body, then receive exact X plugs and R2 roots.
