...
`ramp` is an independently authored floor-to-mat transition for an original roofed female tile edge. Its width is an integer number of unit cells, its rise is tile thickness `t`, and its finished positive-Y run stays 50 mm. Each cell has one matching female pocket at its half-unit centre. The ramp receives a tile's north male edge at Y=0; rotating the part for placement doesn't change which edge is male or female.

The profile is one filled wedge with a 10 mm carrier run at the high edge and a floor-tangent rounded nose. Free exterior edges are R2, the mating-boundary underside transition is R1, and the shared tile-joint tool cuts the female pocket. At standard 60/13 dimensions that pocket is 6.1 mm deep. Separate ramps meet without overlap, while a multi-cell ramp is one continuous solid.
