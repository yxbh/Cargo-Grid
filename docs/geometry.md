...
Logical PETG model/base slot 1 and PLA interface slot 2 are explicit. Gap, interface-layer count and interface spacing are also explicit parameters; they are not calibrated material-release values. Roof support cannot be combined with stacks, catalogues, accessories or full-height joints.

### Slice mode and first-layer defaults

Physical nozzle assignment is automatic unless an explicit `nozzle_map`/`--roof-nozzles` request selects Custom mapping. In the tested native dialect, `Auto For Flush` is Filament-Saving Mode, `Auto For Match` is Convenience Mode, and `Manual` is Custom filament grouping. The distinct `normal(manual)` support type selects enforcer-defined regions and does not force Custom grouping.

`support_on_build_plate_only` is false. Initial support-foot expansion is omitted by default, leaving the slicer's native automatic value; `foot_expansion=0`/`--roof-foot-expansion 0` explicitly requests zero. An observed automatic nozzle assignment is not a locked contract. Explicit process changes are tracked for GUI import, while native-default mapping, build-plate-only and automatic-foot keys are not marked as changes.
