...
def test_accessory_families_finite_and_complete():
    ...
    edge_variants = [
        ...
        if variant.family in {"edge-x", "edge-y", "corner-in", "corner-out"}
    ]
    assert len(edge_variants) == 108
    assert {(variant.edge_outward, variant.complete_edge_holes) for variant in edge_variants} == {
        (10, False),
        (10, True),
        (20, False),
        (20, True),
